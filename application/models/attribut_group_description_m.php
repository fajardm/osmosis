<?php
	Class attribut_group_description_m extends CI_Model{
        ...
        function createNew($id,$name,$lg){
            ...
            if($this->db->insert('cms_attribut_group_description', $data)){
                return true;
            }else{
        ...
        function getOne($id=''){
			$this->db->select("*");
			$this->db->from("cms_attribut_group_description");
            $this->db->join("cms_language","cms_attribut_group_description.language_id=cms_language.language_id");
			$this->db->where("cms_attribut_group_description.agd_id",$id);
            if($hasil = $this->db->get()){
                if($hasil->num_rows() > 0){
                    $data = $hasil->result();
                }else{
